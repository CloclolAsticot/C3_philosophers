...
/*   Updated: 2025/07/22 16:46:30 by csavreux         ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
void clean_all(t_philo *philos, t_config * config)
{
	...
	pthread_mutex_destroy(&config->stop_sim_mutex);	// clean death_bool_mutex
	pthread_mutex_destroy(&config->satiated_philos_counter_mutex);
	if (philos != NULL)
		free(philos);
}
